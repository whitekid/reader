...
// .wrangler/tmp/bundle-3hDXNw/checked-fetch.js
...
// .wrangler/tmp/bundle-3hDXNw/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-3hDXNw/middleware-loader.entry.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-3hDXNw/middleware-insertion-facade.js
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
async function getArticleByUrl(db, url) {
  const result = await db.prepare("SELECT * FROM articles WHERE url = ?").bind(url).first();
  return result || null;
}
__name(getArticleByUrl, "getArticleByUrl");
async function getUnreadArticles(db, limit = 50) {
  const result = await db.prepare("SELECT * FROM articles WHERE is_read = 0 ORDER BY created_at DESC LIMIT ?").bind(limit).all();
  return result.results || [];
}
__name(getUnreadArticles, "getUnreadArticles");
...
async function markAsRead(db, id) {
  const result = await db.prepare("UPDATE articles SET is_read = 1, read_at = CURRENT_TIMESTAMP WHERE id = ?").bind(id).run();
...
<body>
  <div class="toolbar">
    <h1 style="margin: 0; font-size: 20px;">\u{1F4DA} Reader</h1>
  </div>
...
__name(renderArticles, "renderArticles");
...
async function extractContent(url) {
  try {
    ...
    return {
      ...
      publishedTime: null,
      // Can be enhanced with meta tag parsing
      wordCount,
      readingTime
    };
  } catch (error) {
...
__name(estimateWordCount, "estimateWordCount");
...
__name(truncateExcerpt, "truncateExcerpt");
...
// src/handlers/reader.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// src/templates/reader.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${escapeHtml2(article.title)} - Reader</title>
  <style>${styles}</style>
</head>
<body>
  <div class="toolbar">
  ...
  <div class="container">
    ...
    <div class="meta">
      ${article.author ? `by ${escapeHtml2(article.author)} \u2022 ` : ""}
      ${article.site_name ? escapeHtml2(article.site_name) : ""} \u2022
      ${article.reading_time} min read
    </div>
...
__name(renderReader, "renderReader");
...
// src/handlers/reader.ts
async function handleReader(request, env, id) {
  try {
...
__name(handleReader, "handleReader");

// src/handlers/markRead.ts
init_checked_fetch();
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
async function handleMarkRead(request, env, id) {
  try {
    ...
    return new Response(
      `Failed to mark article as read: ${error.message}`,
      { status: 500 }
    );
  }
}
__name(handleMarkRead, "handleMarkRead");
